...
import { renderFichaFront, renderFichaBack } from '../utils/canvas'
import { downloadFichaPDF } from '../utils/pdf'
import { useDebounce } from '../hooks/useDebounce'

export function FichaPreview({ data, onDownload }: { data: FichaData; onDownload: () => void }) {
  ...
  const render = useCallback(async () => {
    ...
    try {
      await Promise.all([
        frontRef.current ? renderFichaFront(frontRef.current, debouncedData) : Promise.resolve(),
        backRef.current  ? renderFichaBack(backRef.current,   debouncedData) : Promise.resolve(),
      ])
  ...
  const handleDownload = useCallback(() => {
    ...
    downloadFichaPDF(frontRef.current, backRef.current)
    onDownload()
  }, [onDownload])
